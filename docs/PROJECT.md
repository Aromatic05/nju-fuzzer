...
本节用于说明本项目在实验过程中产生的**结果数据、可视化图表、分析报告以及数据处理相关代码**的组织方式，便于查看、分析与评估。
...
该目录存放实验的**结果分析报告**（Markdown）：

- 报告中对实验结果进行了整理、对比与分析
- 分析过程中引用并嵌入了 `figures/` 目录中的部分图表
- 该部分内容用于对实验结果进行说明和解释

### 4.4 visualization_function/
...
这些程序用于生成 `figures/` 目录中的图像结果。其中：

- `visualize`：对每一份数据进行处理
- `visualize_totally`：对每个配置的十个样例进行横向对比
- `vertical`：对不同配置的同一样例进行纵向对比
...
✅ **测试覆盖**
- `mvn test` 在当前仓库状态下通过（以 CI/本地实际运行结果为准；避免在文档中硬编码测试数量）。
- 单元测试：Executor, CoverageMonitor, Strategies, CorpusManager
- 集成测试：FuzzingEngine, ExecutorHarness
- 端到端测试：FuzzerMain smoke tests

#### 1.2 现阶段实现覆盖（相对该 v2.0 文档的里程碑）

该 ARCHITECTURE 文档记录的是 2025-12 的阶段性里程碑；截至当前仓库状态，闭环中的以下部分已经落地并接入主循环：

- **种子队列**：`SeedQueue` 负责加载初始 seeds 并接收晋升种子。
- **选种与调度**：`SeedPrioritizer`（选种）+ `PowerScheduler`（能量分配）已实现。
- **变异系统**：`Mutator` 接口 + `AflHavocMutator` + `MutatorFactory` 按 `SeedType` 路由的格式变异器已实现。
- **全局覆盖数据库**：`CoverageDB` 已实现，并可为 `favored/rarity/redundant` 等信号提供支持。
...
// 下面是“概念性伪代码”，用于说明组件关系；实际构造与参数以源码为准。
FuzzStats stats = new FuzzStats(targetSpec.tid());
SeedQueue queue = new SeedQueue();
SeedPrioritizer prioritizer = new SeedPrioritizer();
PowerScheduler scheduler = new PowerScheduler(stats);
MutatorFactory mutatorFactory = new MutatorFactory(queue.getSeeds());
...
try {
    while (shouldContinue()) {
        ...
        // 1) 选择种子
        Seed parent = prioritizer.pick(queue.getSeeds());
        
        // 2) 计算能量
        int energy = scheduler.assignEnergy(parent);
        
        // 3) 生成变异
        Mutator mutator = mutatorFactory.createMutator(parent);
        for (Iterator<Testcase> it = mutator.mutate(parent, energy); it.hasNext(); ) {
            Testcase tc = it.next();
            
            // 4) 构造输入（TargetCommand/输入模式由 targetSpec + tc.data 决定）
            ExecInput input = /* ExecInput.of(cmd, stdinDataOrNull, timeout, outDir) */ null;
            ...
            if (crashOracle.isCrash(er.run())) {
                corpus.saveCrash(tc, er, parent);
                stats.recordCrash();
                continue;
            }
            
            // 7) 更新全局覆盖（示意：实际以 FuzzingEngine 内部实现为准）
            // CoverageDB.UpdateResult upd = covDb.update(seedId, diffResult, inputSize, execTimeNanos);
            
            // 8) 保存有趣的输入（示意）
            // if (upd.isInteresting()) {
            //     Seed child = corpus.saveInteresting(tc, er, parent);
            //     queue.addSeed(child);
            //     stats.recordNewPath();
            // }
            ...
            // 9) 更新统计
            stats.recordExec(er.run().execTimeNanos());
        }
    }
} finally {
    // ===== 清理资源（detach SHM 一次）=====
...
// 下面是“概念性伪代码”，用于说明闭环与组件关系；实际构造与参数以源码为准。
SeedQueue seedQueue = new SeedQueue();
seedQueue.loadInitialSeeds(/*seedsDir*/, /*defaultSeedType*/);
SeedPrioritizer prioritizer = new SeedPrioritizer();
FuzzStats stats = new FuzzStats(/*tid*/ "DEMO");
PowerScheduler scheduler = new PowerScheduler(stats);
MutatorFactory mutatorFactory = new MutatorFactory(seedQueue.getSeeds());
...
try {
  while (shouldContinue()) {
    ...
    ExecResult er = harness.execute(/* ExecInput.of(cmd, stdinDataOrNull, timeout, outDir) */ null);
            ...
            // 7) persist interesting input and enqueue as new seed (示意)
            // if (upd.isInteresting()) {
            //   Seed child = corpus.saveInteresting(tc, er, parent);
            //   seedQueue.addSeed(child);
            //   stats.recordNewPath();
            // }
