...
> 截止日期：2026-01-13
>
> 范围：本文件整合 [docs/DevLog/](../DevLog/) 下的阶段开发日志、[docs/CoverageMonitor/DevLog.md](../CoverageMonitor/DevLog.md) 的迭代记录，以及 [docs/mutate/Problems/](../mutate/Problems/) 下对各 Mutator 的审查问题清单。
>
...
这里给出“按迭代推进”的进度安排模板，并用已完成内容回填，方便你后续继续延展：

- **Iteration 1-3（基础覆盖监控 → 引擎集成 → 统计与 corpus）**：已完成
  - 输出：SHM bitmap 读取、差分策略组合、FileCorpusManager、FuzzStats/StatusPrinter、集成测试。
- **Iteration 4（边级别覆盖 → CoverageDB → 调度信号）**：已完成（2025-12-22）
  - 输出：EdgeSet、CoverageDB、CoverageEx、CoverageMonitorEx。
- **Iteration 5（长跑稳定性与 IO/统计落盘）**：已完成（2025-12-31）
  - 输出：execLogs 策略、curve.csv、tmpfs 输入策略、flush 降频。
- **Iteration 6（Mutate 系统化：接口、工厂路由、通用 Havoc）**：已完成（2026-01-08）
- **Iteration 7（结构/语法感知 Mutator 质量提升）**：进行中（由 Problems 文档驱动）
  - 目标：提升样本“可解析率/可深入率”，减少被解析器早拒的比例。
...
### 5.2 基础 monitor 无法产出 edge-level，CoverageDB 更新跑不起来

- **问题**：基础 SHM 路径下 only-bitmap 判定导致引擎侧无法得到边集合（hit/new）。
- **影响**：CoverageDB 的 `update()` 无触发，调度信号缺失。
- **解决**（2025-12-24）：
  - 让 `ShmCoverageMonitor` 升级实现扩展接口并提供 `afterRunEx()`。
  - 使用 `CoverageDiffStrategyEx.wrap(diffStrategy, mapSize)` 从 bitmap 提取 `hitEdges/newEdges`（将非零 byte 下标视为 edge index）。

### 5.3 interesting 晋升误判：局部差分导致“假新边”
...
### 5.4 长跑 IO 爆炸：exec-logs 小文件/空文件占满 inode 与磁盘
...
### 5.5 `.cur_input` 临时输入策略：避免落盘与隐式退化

- **问题**：FILE 模式需要复用 `.cur_input`，但默认写磁盘会加剧 IO；若 tmpfs 不可用，隐式回退可能掩盖性能问题。
- **影响**：吞吐下降、长跑噪声增加。
- **解决**（2025-12-31）：
  - 默认写入 `/dev/shm`（tmpfs）。
  - 引入 `-Dnju.fuzzer.requireTmpfsInputs=true`：tmpfs 不可用时 fail-fast，避免默默退化。

### 5.6 crash 判定：非 0 退出码不等价于 crash
...
## 6. Mutate 系统开发（2026-01-08）

### 6.1 核心框架：接口、工厂路由、算子库、Havoc 兜底
...
## 7. Mutator 审查问题（Problems 汇总）

本节将 `docs/mutate/Problems/` 中的审查点归纳为“难题/风险”与“建议修复方向”，用于驱动下一阶段（Iteration 7）的质量提升。

### 7.1 共性问题（跨格式）

- **长度/计数/偏移的耦合一致性**：只改字段不改数据，极易导致解析器早拒。
- **顺序约束**：PNG/JPEG 这类有强顺序约束，swap/delete/duplicate 若不守约束会大幅降低有效样本比例。
- **语法闭合与转义**：XML/Lua/C++ mangled 都有严格闭合/转义/配对要求，不处理会导致大量样本无法进入目标深层逻辑。

### 7.2 分格式关键问题摘录

- **PNG（PngMutator）**
  - 长度字段变异不同步修改数据区大小，CRC 修复也随之失效 → 建议实现 resize + 重算 CRC 的“耦合变异”。
  - Chunk 顺序约束未强制（PLTE 必须在 IDAT 前等）→ 建议引入顺序权重/约束检查。

- **ELF（ElfMutator）**
  - Header 必需字段保护不足（e_ident、e_ehsize 等）→ 建议跳过 e_ident 或引入 essentials 修复。
  - Offset/Count 变异不维护一致性（未同步 size/count）→ 建议引入 coupled field 策略。

- **JPEG（JpegMutator）**
  - “长度欺骗”攻击写法导致多数情况下遇到 EOF 直接报错，效果打折 → 建议补充“声明长度 < 实际数据”的错位型攻击。
  - Segment 顺序约束未强制（DQT/DHT 必须在 SOS 前）→ 建议顺序权重约束。

- **PCAP（PcapMutator）**
  - incl_len/orig_len/snaplen 的一致性与攻击解释需统一（当前注释与实现不一致）→ 建议把攻击类型拆成：错位型（incl_len < 实际写入）与 EOF 型（incl_len > 实际写入），并确保 orig_len 非负。

- **XML（XmlMutator）**
  - 未使用树结构，标签名变异不保持开闭配对，属性注入未转义 → 建议：标签配对映射 + 属性转义 + 逐步迁移到 TreeBuilder/MutationStrategy。

- **Lua（LuaMutator）**
  - KEYWORD_REPLACEMENTS 含大量非法语法替换（global/func/!=/! 等）→ 建议替换集合收敛为“语法合法优先”，破坏性替换低概率保留。
  - 运算符替换不区分一元/二元，字符串注入不处理转义/闭合 → 建议按元数分类并增加 escape。

- **C++ Mangled（CxxMutator）**
  - 长度前缀与名称不同步，模板 `I...E` 闭合概率过低，替换序号可能越界 → 建议：长度-名称同步变异、闭合比例控制、替换序号受限。
...
- `docs/DevLog/2026-01-08-mutate-core.md`
  - Mutator/Factory/Ops/Havoc 兜底与设计决策。

- `docs/DevLog/2026-01-08-mutate-binary.md`
  - PNG/ELF/JPEG/PCAP 的结构扫描、字段/块变异与约束修复。

- `docs/DevLog/2026-01-08-mutate-grammar.md`
...
## 9. 下一步（从日志与 Problems 推导的自然演进路线）

- Mutator 质量提升（Iteration 7）：
  - 长度/偏移/计数“耦合变异”与顺序约束，提升有效样本比例。
  - XML 迁移到树结构，Lua 替换集合与转义修复，C++ mangled 同步长度。
- 选择策略演进：
  - MOpt 自适应权重与覆盖反馈驱动的算子选择。
  - 字典词频统计与动态优先级。
- 格式扩展：
  - 二进制：PDF、PE/COFF 等。
  - 语法：JSON/SQL/HTML 等。
